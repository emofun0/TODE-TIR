...
class TodeTIR(nn.Module):
    def __init__(self, lambda_val = 1, res = True):
        super(TodeTIR, self).__init__()

        self.encoder = SwinTransformer(patch_size=2, in_chans= 5, embed_dim=24)
    ...
    def forward(self, img, depth, tir, **kwargs):
        n, h, w = depth.shape
        depth = depth.view(n, 1, h, w)
        tir = tir.view(n, 1, h, w)

        # x = self._netin(torch.cat((img, depth), dim=1))
        encoder_x = self.encoder(torch.cat((img, depth, tir), dim=1))
...
if __name__ == '__main__':
    ...
    model = TodeTIR.build(lambda_val=1)
    x = torch.rand(2, 3, 240, 320) # RGB image
    y = torch.rand(2, 1, 240, 320) # Depth image
    z = torch.rand(2, 1, 240, 320) # TIR14 image
    bins, pred = model(x,y,z)
